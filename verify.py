...
# 从命令行获取目标图标名称
if len(sys.argv) < 2:
    print("Usage: python verify.py <icon_name> (e.g., python verify.py jump2-1)")
    sys.exit(1)
...
def detect_icon(template_filename, capture_interval=1, num_captures=100):
    ...
    if os.path.exists(os.path.join('traindata', opposite_icon)):
        class_names = [icon_name, opposite_icon]  # 例如 ['jump1-1', 'jump1-0']
    else:
        class_names = [icon_name, 'other']  # 例如 ['jump2-1', 'other']
...
if __name__ == "__main__":
    ...
    template_filename = f"{sys.argv[1]}.png"  # 从命令行参数获取图标文件名
