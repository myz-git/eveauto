...
def find_and_set_jump_end(template, width, height, clf, scaler, max_attempts=10,offset_x=0,offset_y=0,region=None,):
    """查找[设置为终点]"""  
    ...
    while attempts < max_attempts:        
        ...
        if max_val > 0.8:
            ...
            if predict_icon_status(icon_image, clf, scaler):
                ...
                return True
        attempts += 1
        print(f"Attempt {attempts}/{max_attempts} 未能确定终点, Retrying...")
        scollscreen()  
        time.sleep(1)
...
def main():
    ...
    time.sleep(2)  # 等待窗口打开
    # 设置终点
    if find_and_set_jump_end(template_gray_zhongdian2, w_zhongdian2, h_zhongdian2, clf_zhongdian2, scaler_zhongdian2,5,0,0,region2):
        pyautogui.leftClick()
