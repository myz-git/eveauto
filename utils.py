...
def find_txt_ocr(txt: str, max_attempts=5, region=None, allow_scroll=True) -> bool:
    ...
    log_message("WARNING", f"文本[{txt}] 未找到，尝试次数={max_attempts}")
    return False
...
@dataclass
class CnnConfig:
    template_threshold: float = 0.85  # 模板匹配阈值
...
def find_icon_cnn(
    ...
    # 读取模板
    template_path = os.path.join(icon_dir, f"{icon}-1.png")
...
def safe_find_icon(
    ...
    for attempt in range(max_attempts):
        ...
        if allow_scroll:
            # 兼容你之前两种命名
            if "scrollscreen" in globals():
                scrollscreen()
            elif "scollscreen" in globals():
                scrollscreen()
        time.sleep(0.25)
